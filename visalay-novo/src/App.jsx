...
// ════════════════════════════════════════════════════════════════════════════
// PAINEL MASTER (MODO SIMULAÇÃO DE TESTES COMPLETO)
// ════════════════════════════════════════════════════════════════════════════
export default function PainelMaster() {
  // Simulação de estados externos que viriam por Props
  const [abaAtivaSuper, setAbaAtivaSuper] = useState('dashboard');
  const [ativosEmCustodiaTSEA, setAtivosEmCustodiaTSEA] = useState([
    { status: 'EM CUSTÓDIA' }, { status: 'EM CUSTÓDIA' }, { status: 'DEVOLVIDO' }
  ]);
  ...
  // Dados mocados para as janelas de setores do Dashboard
  const dashboardData = {
    Soldagem: [
  ...
  // ── Mocks locais para simular as listagens locais ──
  const [usuarios, setUsuarios] = useState([
    { nome: "Marcos Oliveira", cpf: "11122233344", setor: "SOLDA", tipo: "OPERADOR", cartao_operador: true },
    { nome: "Ana Costa", cpf: "55566677788", setor: "MONTAGEM", tipo: "OPERADOR", cartao_operador: false },
    { nome: "Sérgio Almoxarife", cpf: "99988877766", setor: "ALMOXARIFADO", tipo: "ALMOXARIFE", cartao_operador: true }
  ]);
  const [cartoes, setCartoes] = useState([
    { user_cpf: "11122233344", codigo_uid: "A3F2B8C1" },
    { user_cpf: "99988877766", codigo_uid: "E4D5C6B7" }
  ]);
  ...
  const criarUsuario = () => {
    ...
    setSalvando(true);
    setTimeout(() => {
      setUsuarios(p => [...p, { nome: form.nome, cpf: form.cpf, setor: form.setor, tipo: form.tipo, cartao_operador: false }]);
      showMsg('sucesso', 'Usuário cadastrado com sucesso (Simulado)!');
      setForm({ cpf: '', nome: '', senha: '', tipo: 'OPERADOR', setor: 'MONTAGEM' });
      setSalvando(false);
    }, 600);
  };

  const vincularCartao = () => {
    if (!formNFC.user_cpf || !formNFC.codigo_uid) {
      showMsg('aviso', 'Informe o CPF do usuário e o UID do cartão.'); return;
    }
    setSalvandoNFC(true);
    setTimeout(() => {
      setCartoes(p => [...p, { user_cpf: formNFC.user_cpf, codigo_uid: formNFC.codigo_uid }]);
      setUsuarios(p => p.map(u => u.cpf === formNFC.user_cpf ? { ...u, cartao_operador: true } : u));
      showMsg('sucesso', 'Cartão vinculado com sucesso (Simulado)!');
      setFormNFC({ user_cpf: '', codigo_uid: '' });
      setSalvandoNFC(false);
    }, 600);
  };

  const logout = () => alert("Sessão encerrada (Modo de Teste)");
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#fafafa', fontFamily: 'sans-serif' }}>
      
      {/* Container de desfoque controlado se houver modal ativo */}
      <div className="layout-container" style={{ filter: setorSelecionado ? 'blur(4px)' : 'none', transition: 'filter 0.3s', display: 'flex', width: '100%' }}>

        {/* ── SIDEBAR ── */}
        <aside className="sidebar no-print" style={{ width: '260px', background: TSEA.preto, color: 'white', padding: '20px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ textAlign: 'center', borderBottom: `3px solid ${TSEA.vermelho}`, paddingBottom: '15px', marginBottom: '20px' }}>
            ...
            <small style={{ color: TSEA.cinzaBorda, fontSize: '10px' }}>MODO TESTE INDEPENDENTE</small>
          </div>
            ...
            {navItems.map(({ key, label, icon }) => (
              <button
                ...
                style={{
                  ...
                  border: 'none', padding: '12px 15px', color: 'white', borderRadius: '4px',
        ...
        {/* ── CONTEÚDO ── */}
        <main className="content-main" style={{ flex: 1, padding: '30px', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
                {Object.keys(dashboardData).map(setor => (
                  ...
                  >
                    <h4 style={{ color: TSEA.cinzaEscuro, marginBottom: '10px', marginTop: 0 }}>{setor}</h4>
              ...
              {/* Bloco do Gráfico de Cores */}
              <div style={{ ...card, display: 'flex', alignItems: 'center', gap: '40px', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                  <h4 style={{ marginBottom: '15px', marginTop: 0 }}>Status Geral do Inventário</h4>
                  <div style={{ 
                    width: '180px', 
                    height: '180px', 
                    borderRadius: '50%', 
          ...
          {abaAtivaSuper === 'criar_usuario' && (
            <div style={card}>
              ...
              <button onClick={criarUsuario} disabled={salvando} style={{ ...btnPrimary(salvando), marginTop: '24px' }}>
                {Icons.check} {salvando ? 'Salvando...' : 'Cadastrar Usuário'}
              </button>
            </div>
          )}
      ...
      {/* ── MODAL DE DETALHES DO SETOR (Montado fora para aplicar o backdrop-filter) ── */}
      {setorSelecionado && (
        <div style={{
          ...
          <div style={{
            ...card, width: '600px', maxHeight: '80vh', overflowY: 'auto',
            ...
            <h3 style={{ marginBottom: '20px', marginTop: 0, color: TSEA.preto }}>Colaboradores Ativos: {setorSelecionado}</h3>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {dashboardData[setorSelecionado] ? (
                dashboardData[setorSelecionado].map((func, idx) => (
                  <div key={idx} style={{ padding: '15px', background: TSEA.cinzaClaro, borderRadius: '6px' }}>
                    <div style={{ fontWeight: '800', fontSize: '15px', color: TSEA.vermelho, marginBottom: '8px' }}>
                      {func.nome}
                    </div>
                ...
                <p style={{ color: '#888', fontStyle: 'italic' }}>Nenhum dado cadastrado para este setor.</p>
              )}
            </div>
          </div>
        </div>
